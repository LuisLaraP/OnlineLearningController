...
class Controller:
	...
	def __init__(self, settings, environment, logger):
		...
		self.critic = Critic('critic', self.settings['actor'], self.action, self.state, self.isTraining)
		...
		self.criticTarget = Critic('critic_target', self.settings['actor'], self.action, self.state, self.isTraining)
